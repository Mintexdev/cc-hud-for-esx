...
local function hasHarness()
    ...
    local hasHarness = exports['qb-smallresources'] and exports['qb-smallresources']:HasHarness()
    if hasHarness then
        _harness = true
    else
        _harness = false
...
RegisterNUICallback('GetImage', function(_, cb)
    print("GetImage")
    print(Config.Image)
    cb(Config.Image)
end)
...
local function getFuelLevel(vehicle)
    ...
    if (updateTick - lastFuelUpdate) > 2000 then
        ...
        if Config.FuelScript and exports[Config.FuelScript] and exports[Config.FuelScript].GetFuel then
    ...
    if pdata and pdata.accounts then
        for _, acc in ipairs(pdata.accounts) do
            if acc.name == 'money' or acc.name == 'cash' then
                cashAmount = acc.money or 0
            elseif acc.name == 'bank' then
                bankAmount = acc.money or 0
            end
        end
    end
end
